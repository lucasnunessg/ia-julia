        ...
        if (mode === 'caiu-oab') {
          ...
          const response = await openai.chat.completions.create({
            ...
            messages: [
              {
                role: "user",
                content: `Você é um COPISTA. Abaixo estão trechos de um material de estudo, extraídos automaticamente ao redor de marcações como '*CAIU NA OAB 43*' (o número indica o exame da OAB em que o tema caiu). Cada trecho indica a página e os trechos são separados por '====='.\n\nSua ÚNICA tarefa é copiar, PALAVRA POR PALAVRA, o conteúdo de cada trecho ao qual a marcação se refere. REGRAS ABSOLUTAS:\n- É PROIBIDO usar qualquer conhecimento próprio. NÃO cite artigos da Constituição ou de leis de memória. Se uma frase não estiver escrita literalmente no trecho abaixo, ela NÃO pode aparecer na sua resposta.\n- Copie desde o início do tópico/seção em que a marcação aparece (ex.: desde o título '1.2 Poder Constituinte Derivado') até o fim do assunto, incluindo definições, listas e itens — exatamente como estão escritos no trecho.\n- Se a marcação estiver colada a um artigo de lei transcrito NO PRÓPRIO trecho, copie esse artigo como está no trecho.\n- Se houver mais de uma marcação no mesmo tópico, copie o tópico uma única vez, listando todos os exames no cabeçalho.\n- Apenas limpe ruídos de extração: remova cabeçalhos/rodapés repetidos (nome, CPF, IP, '@metodovde', número de página solto) e junte palavras quebradas.\n\nFormato de saída para cada item:\n\n⚖️ CAIU NA OAB <exame(s)> — Página <n>\n<cópia literal do conteúdo do trecho>\n\n${snippets}`
              }
            ],
          })
